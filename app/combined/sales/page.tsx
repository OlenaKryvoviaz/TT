...
export default function CombinedSales() {
  ...
  const getInitialTab = (): 'personal' | 'studying' | 'corporate' => {
  ...
  const [activeTab, setActiveTab] = useState<'personal' | 'studying' | 'corporate'>(getInitialTab());
  ...
  const personalPlans = [
    {
      ...
      yearly: {
        price: '$9.99',
        priceUnit: '/month',
        subtitle: 'Billed annually at $119.88',
      },
      popular: false,
      isSingleUse: false,
    },
    {
      ...
      yearly: {
        price: '$24.95',
        priceUnit: '/month',
        subtitle: 'Billed annually at $299.40',
      },
      popular: true,
      isSingleUse: false,
    },
  ...
  const personalFeatures = [
    { name: "One-time payment", singleUse: true, limited: false, full: false },
    { name: "Single download access", singleUse: true, limited: false, full: false },
    { name: "No subscription", singleUse: true, limited: false, full: false },
    { name: "Unlimited edits", singleUse: false, limited: true, full: true },
    { name: "Unlimited downloads", singleUse: false, limited: true, full: true },
    { name: "Multi-format conversion", singleUse: true, limited: true, full: true },
    { name: "No additional software", singleUse: false, limited: true, full: true },
    { name: "Edit text and images in PDF", singleUse: false, limited: false, full: true },
    { name: "Organize and reorder PDF pages", singleUse: false, limited: false, full: true },
    { name: "Protect PDF with passwords", singleUse: false, limited: false, full: true },
    { name: "Use PDF Guru on mobile device", singleUse: false, limited: false, full: true }
  ];

  const educationFeatures = [
    { name: "Unlimited edits", student: true, teacher: true },
    { name: "Edit text and images in PDF", student: true, teacher: true },
    { name: "Unlimited downloads", student: true, teacher: true },
    { name: "Organize and reorder PDF pages", student: true, teacher: true },
    { name: "Multi-format conversion", student: true, teacher: true },
    { name: "Protect PDF with passwords", student: true, teacher: true },
    { name: "No additional software", student: true, teacher: true },
    { name: "Use PDF Guru on mobile device", student: true, teacher: true },
    { name: "Color-coded annotations", student: true, teacher: false },
    { name: "Progress tracker", student: true, teacher: false },
    { name: "Batch converter", student: false, teacher: true },
    { name: "Quiz templates", student: false, teacher: true },
  ];
  ...
  const renderPersonalTable = () => {
    const plansToShow = showSingleUsePlan ? [singleUsePlan, ...personalPlans] : personalPlans;
    const fullPlanIndex = showSingleUsePlan ? 2 : 1; // Index of the "full" plan which is popular
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              {plansToShow.map((plan) => {
                ...
                return (
                  <th key={plan.id} className={`p-4 relative ${plan.popular ? 'bg-amber-50' : ''}`}>
                    ...
                    <div className={`flex flex-col items-center gap-2 ${plan.popular ? 'mt-6' : ''}`}>
                      ...
                      <div className="text-center">
                        <div className="text-2xl font-bold text-gray-900">
                          {pricing.price}
                          {pricing.priceUnit && <span className="text-sm font-normal text-gray-500">{pricing.priceUnit}</span>}
                        </div>
                        {pricing.subtitle && <div className="text-xs text-gray-500">{pricing.subtitle}</div>}
                      </div>
                    </div>
                  </th>
                );
          ...
          <tbody>
            {personalFeatures.map((feature, index) => (
              <tr key={index} className="border-b border-gray-100 last:border-0">
                ...
                {showSingleUsePlan && (
                  <td className={`p-4 text-center ${selectedPlan === 'single-use' ? 'bg-indigo-50' : ''}`}>
                    {feature.singleUse ? (
                ...
                <td className={`p-4 text-center ${selectedPlan === 'full' ? 'bg-indigo-50' : ''} ${selectedPlan !== 'full' && plansToShow[fullPlanIndex]?.popular ? 'bg-amber-50' : ''}`}>
                  {feature.full ? (
                    <svg className="w-5 h-5 text-indigo-600 mx-auto" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                    </svg>
        ...
        <div className="p-4 bg-gray-50 text-xs text-gray-600 space-y-2">
          {selectedPlan === 'single-use' ? (
            <p>
              By continuing, you agree to make a one-time payment of $0.59 for single download access to your document.{' '}
              <span className="font-semibold">No subscription required.</span> This is a one-time purchase with instant access.
            </p>
          ) : (
            <>
              <p>
                By continuing, you agree that your subscription will be auto-renewed at the price of{' '}
                {billingCycle === 'yearly' ? '$24.95 per month (billed annually)' : '$49.99 each month'} at the end of the 7-day intro period unless you cancel your subscription.{' '}
                <a href="#" className="text-indigo-600 hover:underline">Learn more in the Subscription Policy.</a>
              </p>
              <p className="font-semibold">
                30-day money-back guarantee.{' '}
                <span className="font-normal">Our goal is customer satisfaction</span>
              </p>
            </>
          )}
        </div>
      ...
      <>
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
          <table className="w-full">
            ...
            <tbody>
              {educationFeatures.map((feature, index) => (
                <tr key={index} className="border-b border-gray-100 last:border-0">
                  <td className="p-4 text-sm text-gray-700">{feature.name}</td>
                  <td className={`p-4 text-center ${selectedPlan === 'student' ? 'bg-indigo-50' : ''}`}>
                    {feature.student ? (
                      <svg className="w-5 h-5 text-indigo-600 mx-auto" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                      </svg>
                  ...
                  <td className={`p-4 text-center ${selectedPlan === 'teacher' ? 'bg-indigo-50' : ''} ${selectedPlan !== 'teacher' && studyingPlans[1].popular ? 'bg-amber-50' : ''}`}>
                    {feature.teacher ? (
                      <svg className="w-5 h-5 text-indigo-600 mx-auto" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                      </svg>
          ...
          <div className="p-4 bg-gray-50 text-xs text-gray-600 space-y-2">
            <p>
              By continuing, you agree that your subscription will be auto-renewed at the price of{' '}
              {billingCycle === 'yearly' ? '$24.95 each year' : '$49.99 each month'}{' '}
              at the end of the 7-day intro period unless you cancel your subscription.{' '}
              <a href="#" className="text-indigo-600 hover:underline">Learn more in the Subscription Policy.</a>
            </p>
            <p className="font-semibold">
              30-day money-back guarantee.{' '}
      ...
      <main className="max-w-7xl mx-auto px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            {/* Billing Cycle Toggle - Only show for non-corporate tabs */}
            {activeTab !== 'corporate' && (
            ...
            {/* Plans Tables */}
            <div className="mb-8">
              {activeTab === 'personal' && renderPersonalTable()}
              {activeTab === 'studying' && renderEducationTable()}
              {activeTab === 'corporate' && renderCorporateView()}
            </div>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-8">
              ...
              {/* Continue Button */}
              <button 
                onClick={() => router.push(`/combined/payment?plan=${selectedPlan}&billing=${billingCycle}`)}
